...
module.exports = class FiringLogger extends Components.Base{
    constructor(props){
        ...
        this.options = {
            fsState: {
                recurrentUpdateLimit: (process.env.KILN_LOG_DATAPOINT_FS_INTERVAL_LIMIT_SECONDS || 600) * 1000,
                options: {
                    include: [
                        {
                            key: "localId"
            ...
            if (isFiring){
                ...
                this.setState({
                    ...
                    kilnLog: {
                        ...
                        firing_error: null,
                        starting_fahrenheit_temperature: this.global.Kiln.thermoSensor.average,
                        created_at: Date.now(),
                        updated_at: Date.now()
                    },
        ...
        this.stateChanged.on("kilnLog", (kilnLog)=>{
            ...
            this.setState((state)=>{
                ...
                if (!this.global.Authentication.socketIsAuthenticated){
                    ...
                    this.global.socket.emit("bulk-update-or-create-kiln-logs", [...state.backupKilnLogs, kilnLog], (error)=>{
                        if (error){
                            console.log(error)
                            state.backupKilnLogs.push(kilnLog)
                            addedToBackup = true
        ...
        this.globalChanged.on("Authentication.socketIsAuthenticated", (socketIsAuthenticated)=>{
            if (socketIsAuthenticated){
                ...
                if (this.state.backupKilnLogs.length > 0){
                    ...
                    this.global.socket.emit("bulk-update-or-create-kiln-logs", this.state.backupKilnLogs, (error)=>{
                        if (error){
                            console.log(error)
                        } else {
                            this.setState({
